<header class="header">
    <div class="box">
        <div class="header__body">
            <div class="header__logo logo"><a href="{{ url('/', app()->getLocale() )}}"> <img
                        src="{{ config('puzzles.options.logo') }}"></div>
            <nav class="header__nav">
                <ul class="header__nav-list">
                    {{-- <li class="header__nav-li"><a class="header__nav-link" href="{{ url('/', app()->getLocale() )}}" target="_self">@lang('header.main')</a></li> --}}
                    <li class="header__nav-li"><a class="header__nav-link"
                                                  href="{{ url(app()->getLocale().'/member_cases') }}"
                                                  target="_self">@lang('header.member_cases')</a></li>
                    <li class="header__nav-li"><a class="header__nav-link"
                                                  href="{{ url(app()->getLocale().'/factor_diagram') }}"
                                                  target="_self">@lang('header.factor_diagram')</a></li>
                    <li class="header__nav-li"><a class="header__nav-link" href="{{ url(app()->getLocale().'/about') }}"
                                                  target="_self">@lang('header.about')</a></li>
                    <li class="header__nav-li"><a class="header__nav-link" href="{{ url(app()->getLocale().'/news') }}"
                                                  target="_self">@lang('header.news')</a></li>
                    <li class="header__nav-li"><a class="header__nav-link"
                                                  href="{{ url(app()->getLocale().'/literature') }}"
                                                  target="_self">@lang('header.literature')</a></li>
                    <li class="header__nav-li"><a class="header__nav-link" href="{{ url(app()->getLocale().'/faq') }}"
                                                  target="_self">@lang('header.faq')</a></li>
                </ul>
            </nav>
                ...
                @foreach (config('app.available_locales') as $locale)
                    ...
                    @if(ctype_digit(substr(url()->current(), strrpos(url()->current(), '/') + 1)) && (int) substr(url()->current(), strrpos(url()->current(), '/') + 1) > 0 || strpos(url()->current(), csrf_token()) !== false || \Request::route()->getName() == "news_category" || \Request::route()->getName() == "literature_category" || \Request::route()->getName() == "news_show" || \Request::route()->getName() == "news.show"|| \Request::route()->getName() == "member_cases.show")
                        <a class="header__lang"
                           href="{{ route(\Illuminate\Support\Facades\Route::currentRouteName(), [$locale, substr(url()->current(), strrpos(url()->current(), '/') + 1)]) }}">{{ strtoupper($locale) }}</a>
                    @else
                        <a class="header__lang"
            ...
            <div class="header__login">
                ...
                @auth
                    <button>
                        ...
                        @else
                            <div class="user-ico"><img src="{{ asset(Auth::user()->img) }}" alt="User"></div>
                        @endif
                    </button>
                    <a class="logout"
                       href="{{ url(app()->getLocale().'/personal_cabinet') }}">{{Auth::user()->nickname}}</a>
            ...
            <div class="header__search-block">
                <button class="search-btn" id="search-btn-js"><img src="/img/svg/search.svg"
                                                                   alt="@lang('header.placeholder_search')"></button>
