...
class AddForeignKeysToArticleCategoriesForNewsTable extends Migration {
	...
	public function up()
	{
		...
		{
			$table->foreign('article_id', 'FK_articles_categories_for_news_articles')
                ->references('id')
                ->on('articles')
                ->onUpdate('CASCADE')
                ->onDelete('CASCADE');
			$table->foreign('category_for_news_id', 'FK_articles_categories_for_news_categories_for_news')
                ->references('id')
                ->on('categories_for_news')
                ->onUpdate('CASCADE')
                ->onDelete('CASCADE');
		});
	}
